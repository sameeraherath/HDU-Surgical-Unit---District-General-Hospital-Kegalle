...
export const getAuditStatistics = async (filters = {}) => {
  ...
  const response = await apiClient.get(
    `/audit/statistics?${params.toString()}`
  );
  return response.data;
};
...
export const getUserActivityTimeline = async (userId, page = 1, limit = 50) => {
  const response = await apiClient.get(
    `/audit/user/${userId}/timeline?page=${page}&limit=${limit}`
  );
  return response.data;
};

// Get patient activity timeline
export const getPatientActivityTimeline = async (
  patientId,
  page = 1,
  limit = 50
) => {
  const response = await apiClient.get(
    `/audit/patient/${patientId}/timeline?page=${page}&limit=${limit}`
  );
  return response.data;
};
...
export const getCriticalEvents = async (limit = 20, hours = 24) => {
  const response = await apiClient.get(
    `/audit/critical-events?limit=${limit}&hours=${hours}`
  );
  return response.data;
};
...
export const getFailedActions = async (page = 1, limit = 50, hours = 24) => {
  const response = await apiClient.get(
    `/audit/failed-actions?page=${page}&limit=${limit}&hours=${hours}`
  );
  return response.data;
};
